...
public class ArmyModleBuilder {
	...
	private void initArmyTemp() throws BiffException, IOException{
		...
		for(int i=1;i<sheet.getRows();i++){
			...
			template.level = NumberUtils.toInt(sheet.getCell(7,i).getContents().trim());
			template.skill = NumberUtils.toInt(sheet.getCell(8,i).getContents().trim());
			template.icon = sheet.getCell(9,i).getContents().trim();
			template.describe = sheet.getCell(10,i).getContents().trim();
			template.productLimit = NumberUtils.toInt(sheet.getCell(11,i).getContents().trim());
