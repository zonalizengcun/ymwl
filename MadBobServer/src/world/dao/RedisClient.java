...
public class RedisClient {
	...
	public void init() {
		Config worldConf = World.getDefault().getConfig();
 		JedisPoolConfig config = new JedisPoolConfig();
		config.setMinIdle(redis_pool_minIdle);
		config.setMaxIdle(redis_pool_maxIdle);
		config.setMaxTotal(redis_pool_maxTotal);
		if (worldConf.jedisPass == null || worldConf.jedisPass.equals("")) {
			jedisPool = new JedisPool(config, worldConf.jedisIp, worldConf.jedisPort);
		}else
		jedisPool = new JedisPool(config, worldConf.jedisIp, worldConf.jedisPort, Protocol.DEFAULT_TIMEOUT,
				worldConf.jedisPass);
	}
